...
function showLoadingState() {
    ...
    // Apply basic styles for the loading container
    container.style.cssText = `
        position: fixed; top: 50px; left: 50px;;
        background-color: #ffffff; border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.15);
        padding: 40px; z-index: 2147483647; display: flex; justify-content: center; align-items: center;
    `;
    // Simple spinner
    container.innerHTML = `המתן`;
    document.body.appendChild(container);
}

function handleOutsideClick(event) {
    const summaryContainer = document.getElementById('ai-summary-container');
    // If the container exists and the click was outside of it
    if (summaryContainer && !summaryContainer.contains(event.target)) {
        // Check if the click was inside the shadow DOM
        if (event.composedPath().includes(summaryContainer)) return;

        summaryContainer.remove();
        document.removeEventListener('click', handleOutsideClick, true); // Clean up listener
    }
}
...
function displaySummary(summary) {
    ...
    closeButton.style.cssText = `
        position: absolute; top: 10px; left: 15px; font-size: 24px; /* Adjusted for RTL */
...
function createSummarizerButton() {
    ...
    button.addEventListener('click', async () => {
        try {
            ...
            await summarizeText(content, lang).then(displaySummary);
        } catch (error) {
            console.error(error);
            displaySummary(`<p style="color: #ffdddd;">Error: ${error.message}</p>`); // Display error in the box
        }
    });
        ...
        summarizeText(request.content, request.selectedLanguage)
            ...
            .catch(error => {
                ...
                if (error.message.includes('API Key not found')) {
                    ...
                    displaySummary(`<div class="error-message"><p>API Key is missing.</p><p><a href="${optionsUrl}" target="_blank">Please set your API key in the options page.</a></p></div>`);
                } else {
                    displaySummary(`<p style="color: #ffdddd;">Error: ${error.message}</p>`); // Display error
                }
            });
